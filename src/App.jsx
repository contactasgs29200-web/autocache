...
function refineCornersByPixels(ctx, plate, imgW, imgH) {
  ...
  const expectedHpx = Math.round((pw * imgW) / 4.73); // ratio 520×110 mm

  const edgesForBand = (relA, relB) => {
    const c0 = Math.round(relA * sw);
    const c1 = Math.max(c0 + 1, Math.round(relB * sw));

    // Moyenne de luminosité par ligne dans la bande
    const rowAvg = new Float32Array(sh);
    for (let r = 0; r < sh; r++) {
      let sum = 0;
      for (let col = c0; col < c1; col++) sum += getBright(col, r);
      rowAvg[r] = sum / (c1 - c0);
    }
    ...
    // Ancre = max(prHpx, expectedHpx) : pour une voiture de côté, pw est raccourci par
    // la perspective donc expectedHpx sous-estime la hauteur réelle ; prHpx corrige ça.
    const anchorHpx = Math.max(prHpx, expectedHpx);
    const midRow = Math.min(sh - 1, topEdge + Math.round(anchorHpx * 0.50));
    const refBright = smooth[midRow];
    if (refBright < 100) return null; // intérieur trop sombre → topEdge probablement faux

    // Bord inférieur : dernier pixel encore lumineux dans fenêtre ±20 % autour de anchorHpx
    const botMin = Math.min(sh - 1, topEdge + Math.round(anchorHpx * 0.80));
    const botMax = Math.min(sh - 1, topEdge + Math.round(anchorHpx * 1.20));
...
async function detectGptCropCorners(ctx, plate, imgW, imgH) {
  ...
  const mg = 0.12; // marge 12% (plaque remplit ~82% du crop)
...
async function detectPlateCornersByAI(ctx, plate, imgW, imgH) {
      ...
      try {
        ...
        if (raw) {
          ...
          if (detH > 0 && detW > 0 && detW >= detH * 1.5) {
            console.log(`AI crop rough: TL(${result.tl.x.toFixed(3)},${result.tl.y.toFixed(3)}) size=${detW.toFixed(3)}×${detH.toFixed(3)}`);
            // Affinage pixel : scan whiteness pour trouver les vrais bords
            const refined = pixelScanRefinePlate(ctx, result, imgW, imgH);
            if (refined) return refined;
            return result; // pixel scan failed → use Claude rough
          }
  ...
  try {
    ...
    const roughResult = { tl: raw.tl, tr: raw.tr, br: raw.br, bl: raw.bl };
    console.log(`AI full rough: TL(${raw.tl.x.toFixed(3)},${raw.tl.y.toFixed(3)}) size=${detW.toFixed(3)}×${detH.toFixed(3)}`);
    const refined = pixelScanRefinePlate(ctx, roughResult, imgW, imgH);
    if (refined) return refined;
    return roughResult;
  } catch(e) {
...
// Affinage pixel des coins après détection IA.
// Claude donne une position approximative (±10%) ; ce scan trouve les vrais bords
// de la zone blanche/jaune de la plaque avec une précision sub-pixel.
// Fonctionnement :
//  1. Scan horizontal au centre de la plaque → trouver bord gauche/droit de la zone blanche
