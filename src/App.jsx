  ...
  try {
    ...
    for (let y = carImg.height - 1; y >= 0; y--) {
      ...
      for (let x = 0; x < carImg.width; x++) {
        if (data[(y * carImg.width + x) * 4 + 3] > 40) { found = true; break; }
      }
...
async function processPhoto(photoFile, logoImg, adj, bgColor = "#ffffff", enhance = false, headlightPolish = false, useGptAngle = false, floorClean = false, enhancePro = false, bodyPolish = false) {
  ...
  // Étape 2 : crop depuis canvas c (déjà dessiné) + appel /api/corners (existant) sur le crop
  ...
  if (plate.found) {
    try {
      ...
      // Crop depuis le canvas c (résolution complète, déjà en mémoire, pas de rechargement)
      const srcX = Math.round(x0 * c.width),  srcY = Math.round(y0 * c.height);
      const srcCW = Math.round((x1 - x0) * c.width), srcCH = Math.round((y1 - y0) * c.height);
      // Limiter l'envoi à GPT-4o (max 1200px côté long)
      const sc = Math.min(1, 1200 / Math.max(srcCW, srcCH));
      const outW = Math.round(srcCW * sc), outH = Math.round(srcCH * sc);
      const cropCanvas = document.createElement('canvas');
      cropCanvas.width = outW; cropCanvas.height = outH;
      cropCanvas.getContext('2d').drawImage(c, srcX, srcY, srcCW, srcCH, 0, 0, outW, outH);
      const cropB64 = cropCanvas.toDataURL('image/jpeg', 0.92).split(',')[1];
      console.log(`%c[AutoCache] Crop → (${x0.toFixed(3)},${y0.toFixed(3)})→(${x1.toFixed(3)},${y1.toFixed(3)}) ${outW}×${outH}px`, "color:cyan;font-weight:bold");

      // Appel /api/corners (déjà déployé) sur le crop zoomé
      const r = await fetch("/api/corners", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      console.log("[AutoCache] /api/corners sur crop:", zd);
      const fc = zd.corners;
      if (fc && fc.tl && fc.tr && fc.br && fc.bl) {
        // Reconvertir coins du crop (0-1) → image complète (0-1)
        ...
        console.log(`%c[AutoCache] Coins GPT-4o zoomés ✓`, "color:lime;font-weight:bold");
      } else {
        console.error("[AutoCache] /api/corners crop réponse invalide:", zd);
      }
    } catch(e) {
      console.error("[AutoCache] crop+corners échoué:", e.message ?? e);
    }
  }
