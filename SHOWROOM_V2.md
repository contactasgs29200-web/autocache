...
| **2 — Palier Stripe** | ✅ Implémenté : deux abonnements **Pro** (14,90 €/mois, sans showroom) et **Premium** (29,90 €/mois, showroom Photoroom + ombre, **quota 150 photos showroom/mois**). Essai gratuit : **3 photos showroom Photoroom offertes** (à vie). Anciens abonnés du plan unique (`plan="premium"`) : conservent le showroom **local** @imgly, jamais Photoroom. Quota contrôlé **côté serveur** sur `/api/showroom-cutout` (jeton Supabase requis, compteurs `showroom_used` / `showroom_trial_used` incrémentés en base après chaque succès fournisseur). À configurer dans Stripe : produit Premium + 3 prix (hebdo 9,90 € / mensuel 29,90 € / annuel 259,90 €) → variables Vercel `STRIPE_PREMIUM_WEEKLY_PRICE_ID`, `STRIPE_PREMIUM_MONTHLY_PRICE_ID`, `STRIPE_PREMIUM_ANNUAL_PRICE_ID`. | 2–3 jours |
